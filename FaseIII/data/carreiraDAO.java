...
public class TurmaDAO implements Map<String, Turma> {
     ...
     * @throws NullPointerException Em caso de erro - deveriam ser criadas exepções
     *                              do projecto
     */
    @Override
    public boolean containsKey(Object key) {
        ...
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
                Statement stm = conn.createStatement();
                ResultSet rs = stm.executeQuery("SELECT Id FROM turmas WHERE Id='" + key.toString() + "'")) {
            r = rs.next();
        } catch (SQLException e) {
            // Database error!
            e.printStackTrace();
    ...
    private Collection<String> getAlunosTurma(String tid, Statement stm) throws SQLException {
        ...
        } // execepção é enviada a quem chama o método - este try serve para fechar o
          // ResultSet automaticamente
        return alunos;
    }
     ...
     * @param t   a turma
     * @return para já retorna sempre null (deverá devolver o valor existente, caso
     *         exista um)
     * @throws NullPointerException Em caso de erro - deveriam ser criadas exepções
     *                              do projecto
     */
